...
Sql = "SELECT Park, Year, COUNT(DISTINCT SurveyName) AS Surveys, Network, Methodology
FROM GSPE_Surveys
WHERE (NOT (Park IN ('SELA'))) AND (NOT (Park IS NULL))
GROUP BY Park, Year, Network, Methodology
ORDER BY Park, Year"
DF = dbGetQuery(Connection,Sql)

ggplot(DF) +
  geom_bar(aes(x = Year,fill=Methodology)) +
  facet_grid(rows = vars(Park)) +
  scale_y_discrete(breaks = c(1, 2, 3)) +
  scale_x_continuous(breaks = seq(1950, max(DF$Year)+1, by = 2)) +
  theme_minimal() +
  theme(
...
--[Table],
      ...
      ,[Scale]
      ,[Nullable]
FROM [DatabaseColumnsDescriptions] where [table] = 'GSPE' order by [Column]"
DF = dbGetQuery(Connection,Sql)
