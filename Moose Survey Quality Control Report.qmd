...
SurveyName = '2025 YUCH Moose Survey'
...
Survey details appear in Table `r TableCounter`, below.

```{r,echo=FALSE, label="Details"}
Caption = paste("Table ",TableCounter,". Survey details.",sep="")
kable(DashboardDF_t, escape = TRUE, caption = Caption)
TableCounter = TableCounter + 1
```

# Results Summary

Survey results are presented in Table3, below. Please note these results are calculated automatically by the database and may not account for the unique factors that affect a moose survey (logistical or weather problems, whether a sightability correction factor was generated and applied, missing or incomplete data collection, missing units, partially surveyed units, differing methods among flight crews, etc.). Consult the [survey report](`r DashboardDF$ReportLink`).

Table `r TableCounter`. Survey results.

```{r,label="Table of survey summary data", echo=FALSE}
Caption = paste("Table ",TableCounter,". Survey data summary.",sep="")
Summary = dbGetQuery(Connection,paste0("SELECT SurveyName, Park, Year, StartDate, EndDate, N, [Records missing polygons], [Proportion of records certified], [Area of survey (Km^2)], [Area of surveyed units (Km^2)], [Area of survey (Mi^2)], [Area of surveyed units (Mi^2)], 
    [Calves: 100 cows], [Bulls: 100 cows], [Calves: 100 adults], [Minimum density (unadjusted moose/sq. mi.)], [Area_SqMi, counted], [N, counted], [N, not counted], [N, Counted is NULL], [Proportion of units counted], CALF, 
    [Total calves (Calculated)], COW_W_0, COW_W_1, COW_W_2, COW_W_3, COW, [Total cows (Calculated)], YBULL_SF, YBULL_GTSF, SM_BULL, MED_BULL, MED_L_BULL, LG_BULL, BULL_30_40, BULL_30_50, BULL_30_60, 
    BULL_41_50, BULL_GT_50, BULL_GT_60, BULL_GTE_50, BULL_LT_30, BULL_LT_50, YBULL_ALL, BULL_ALL, [Total bulls (Calculated)], UNKNOWN, ADULT, [Total adults (Calculated)], MOOSE, [Total moose (Calculated)], 
    Personnel, Report, DeliverableDataset, Protocol, SurveyUnitSet
FROM     Summary_GSPE
WHERE (SurveyName = '",SurveyName,"')"))
knitr::kable(t(Summary), caption = Caption)
TableCounter = TableCounter + 1
```
...
## QC Test `r QCTestCounter`: Missing records (Empty survey)
